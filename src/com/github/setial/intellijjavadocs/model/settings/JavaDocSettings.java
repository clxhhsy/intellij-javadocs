...
public class JavaDocSettings implements Serializable
{
    ...
    private JavaDocSettings(Element element)
    {
        ...
        if (general != null)
        {
            ...
            generalSettings.setOverriddenMethods(
                Boolean.parseBoolean(general.getChild(OVERRIDDEN_METHODS).getValue()));
            ...
            generalSettings.setVisibilities(
                XmlUtils.getValues(general, VISIBILITIES, VISIBILITY, Visibility.class));
        }
    ...
    public void addToDom(Element root)
    {
        ...
        if(generalSettings.getMode()!=null) {
            ...
            general.addContent(
                    XmlUtils.getElement(VISIBILITIES, VISIBILITY, generalSettings.getVisibilities()));
        }
        Element templates = new Element(TEMPLATES);
        root.addContent(templates);
        templates.addContent(
            XmlUtils.getElement(CLASSES, CLASS, templateSettings.getClassTemplates()));
        templates.addContent(XmlUtils.getElement(CONSTRUCTORS, CONSTRUCTOR,
            templateSettings.getConstructorTemplates()));
        templates.addContent(
            XmlUtils.getElement(METHODS, METHOD, templateSettings.getMethodTemplates()));
        templates.addContent(
            XmlUtils.getElement(FIELDS, FIELD, templateSettings.getFieldTemplates()));
        templates.addContent(
            XmlUtils.getElement(VARIABLES, VARIABLE, templateSettings.getVariables()));
    }
        ...
        throws CloneNotSupportedException
    {
        ...
        try
        {
            ...
            {
                result.generalSettings = (GeneralSettings)BeanUtils.cloneBean(
                    this.generalSettings);
            }
            if (this.templateSettings != null)
            {
                result.templateSettings = (TemplateSettings)BeanUtils.cloneBean(
